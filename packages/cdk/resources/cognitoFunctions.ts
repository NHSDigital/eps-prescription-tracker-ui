...
export class CognitoFunctions extends Construct {
  public constructor(scope: Construct, id: string, props: CognitoFunctionsProps) {
    ...
    const jwks = new LambdaFunction(this, "JwksResources", {
      stackName: props.stackName!,
    ...
    const restApiGateway = new RestApiGateway(this, "RestApiGatewayResources", {
    ...
    new apigateway.BasePathMapping(this, "api-gw-base-path-mapping", {
      ...
      restApi: restApiGateway.restApiGateway
    })

    const tokenResource = restApiGateway.restApiGateway.root.addResource("token")
    tokenResource.addMethod("POST", new apigateway.LambdaIntegration(token.lambda, {
      credentialsRole: restApiGateway.restAPiGatewayRole
    }))

    const jwksResource = restApiGateway.restApiGateway.root.addResource("jwks")
    ...
    if (props.useMockOidc) {
      const mockTokenResource = restApiGateway.restApiGateway.root.addResource("mockToken")
